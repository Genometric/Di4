...
def process(tool, sort_data_path, ref_file_sorted, results_path):
    threads = []
    files = list_files(sort_data_path)
    for x in range(DP):
        t = threading.Thread(target=worker, args=(tool, files[(len(files)/DP)*x:((len(files)/DP)*(x+1))-1], ref_file_sorted, results_path, ))
        threads.append(t)
        t.start()
    for x in range(DP):
        threads[x].join()

def worker(tool, files, ref_file_sorted, results_path):
    for f in files:
        if tool == BEDTOOLS:
